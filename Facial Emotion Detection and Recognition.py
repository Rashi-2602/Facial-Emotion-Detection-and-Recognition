...
H = model.fit(augmentation.flow(trainImage, trainLabels, batch_size = 32), validation_data=(valImage, valLabels), steps_per_epoch=len(trainImage)//32, epochs=5, verbose=1)             
...
loaded_model = load_model("emotion_model.hdf5", compile=False) 
...
face_class = cv2.CascadeClassifier("haarcascade_frontalface.xml")
